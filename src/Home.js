...
function Home() {
    ...
    function setSection(newSection) {
      if (newSection !== currentSection) {
        ...
        if (className === "first panel") {
          ...
          Fade("page8Title", -360);
          FadeAcross("socialContainer", 1000);
          
        } else if (className === "panel ninth-panel") {
          let tl = gsap.timeline();
          tl.from(".madewithLeftColumn", {x: -1000})
          .from(".madewithRightColumn", {x: 1000})
          .to(".madewithLeftColumn", {x: 0, duration: 0.2, ease: "power3.inOut"})
          .to(".madewithRightColumn", {x: 0, duration: 0.2, ease: "power3.inOut"})
        }
      }
    }
  ...
  useEffect(() => {
    ...
    panel9[0].addEventListener("mousemove", function(event) {
      parallaxed(event, "backgroundTitle-9");
    });

    panel4[0].addEventListener("mousemove", function(event) {
      parallaxed(event, "backgroundTitle-4");
    });
    ...
    <div className="App">
      ...
      <section class="panel fourth-panel">
        ...
        <div className="center-button" onClick={()=> {
            window.open("https://firebasestorage.googleapis.com/v0/b/kietdo-580f0.appspot.com/o/resume.pdf?alt=media&token=16620916-df1a-45f0-9476-b0486cdf4e0b", "resume");
          }}>
